<x-app-layout>
    ...
    <div class="relative max-w-7xl mx-auto px-6">
        @if (session('message'))
        <div class="flash-message text-lg font-bold absolute left-[12%] z-10">
            {{ session('message') }}
        </div>
        @endif
        <form method="post" action="{{ route('post.update', $post) }}">
            ...
            <div class="w-full flex flex-col mb-6">
                ...
                <textarea name="content"
                    class="
                        font-jp w-auto py-2
                        bg-transparent border-2 border-neon-blue rounded-md focus:outline-none
                        resize-none glow-deep-blue
                    "
                    id="body" cols="30" rows="5" placeholder="ここに投稿内容を入力してください">{{ old('content', $post->content) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="spotify_track_id" id="spotify_track_id"
                    class="
                        w-96 py-2
                        bg-transparent border-2 border-neon-blue rounded-md focus:outline-none glow-deep-blue
                    "
                    value="{{ old('spotify_track_id', $post->spotify_track_id) }}"
                    placeholder="例: 3Swc58q85zNn907eY070gD">
            </div>
